---
title: "Demonstration of Collaborative, Open and Reproducible Cloud-Based Data Integration, Visualization, Authoring and Data Sharing with R and Quarto via Binder"
...
This word-cloud is only knitted into HTML.

```{r}
decade_to_view <- 1940
library(wordcloud2)
babynames %>% pivot_longer(cols = boy_name:girl_number, names_to = c("sex",".value"), names_sep = "_") %>% filter(decade==decade_to_view) -> termdata
...
par(mar=rep(0, 4))
plot.new()
text(x = 0.5, y = 0.5, paste0("Wordcloud of Top 200 U.S. Baby Names in the ",decade_to_view,"s"), cex = 1.5, font = 2)
...
```{r}
library(DT)
datatable(mammals,caption="Data for Placental Non-Flying Mammals from Ernest (2003). Ecology 84:3402")
```
