...
#### Wordcloud of Name Frequencies in the 2000s

```{r}
library(wordcloud2)
babynames %>% pivot_longer(cols = boy_name:girl_number, names_to = c("sex",".value"), names_sep = "_") %>% filter(decade==2000) -> termdata
data     <- data.frame(word=termdata$name,freq=termdata$number)
data     <- data[order(data$freq,decreasing = TRUE),]
wordcloud2(data = data,size=0.4)
```
